...
test('add json middleware per route instead of globally', async () => {
    ...
    const responseWithMiddleware = await axios.post<any, AxiosResponse<{ one: number }>>(`http://localhost:${backend.port()}/json-middleware`, {one: 1});
    expect(responseWithMiddleware.data.one).toEqual(1)

    const responseWithoutMiddleware = await axios.post(`http://localhost:${backend.port()}/no-json-middleware`, {one: 1});
